...
const clientes = {
    ...
    get : async( data, callback ) => {
        ...
        const dataExist = await file.read( { file : 'clientes', ext : 'json' } );
        const statusCode = dataExist instanceof ErrorFile ? 500 : 200;
        const response =  dataExist instanceof ErrorFile ? { message : 'Datafile error: The file CLIENTES could not be created' } : id ? dataExist.filter( element => element.id === id ) : dataExist;
    ...
    post : async( data, callback ) => {
        ...
        const dataExist = await file.read( { file : 'clientes', ext : 'json' } );
        const newFile = dataExist instanceof ErrorFile ? await file.create( { file : 'clientes', data : [ newRegister ], ext : 'json' } ) : dataExist.filter( element => element.email === email );
        !( dataExist instanceof ErrorFile ) && dataExist.push( newRegister );

        if ( newFile instanceof ErrorFile ) 
            return callback( 500, { message : 'The file CLIENTES could not be created' } );

        if ( newFile.length > 0 ) 
            return callback( 406, { message : 'The register already exist' } );

        const result = newFile.length === 0 && await file.update( { file : 'clientes', data : dataExist, ext : 'json' } );
        
        if ( newFile.length === 0 && result instanceof ErrorFile )
            return callback( 500, { message : 'The file CLIENTES could not be updated' } );
    ...
    put : async( data, callback ) => {
        ...
        if ( !firstName && !lastName && !telefono && !direccion )
            return callback( 400, { message : 'Missing required fields or not valid' } );
        ...
        if ( dataExist instanceof ErrorFile ) 
            return callback( 500, { message : 'The file CLIENTES does not exist' } );
        ...
        const dataUpdate = await file.update( { file : 'clientes', data : dataAvailable, ext : 'json' } );
